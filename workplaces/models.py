...
class Workplace(models.Model):
    ...
    def clean(self):
        ...
        if self.employee and self.desk_number:
            ...
            skills_qs = self.employee.employeeskill_set.values_list('skill__name', flat=True)
            ...
            is_developer = any(s in ['разработчик', 'developer', 'backend', 'frontend'] for s in skills_lower)
            is_tester = any(s in ['тестировщик', 'qa', 'тестирование', 'tester'] for s in skills_lower)
            ...
            try:
                ...
                raise ValidationError({'desk_number': 'Номер стола должен быть числовым для проверки соседей.'})
            ...
            for neighbor in neighbors:
                neighbor_skills_qs = neighbor.employee.employeeskill_set.values_list('skill__name', flat=True)
                neighbor_skills_lower = [s.lower() for s in list(neighbor_skills_qs)]

                neighbor_is_dev = any(
                    s in ['разработчик', 'developer', 'backend', 'frontend'] for s in neighbor_skills_lower)
                neighbor_is_tester = any(
                    s in ['тестировщик', 'qa', 'тестирование', 'tester'] for s in neighbor_skills_lower)

                # Если разработчик и сосед — тестировщик (или наоборот) -> Ошибка
                if (is_developer and neighbor_is_tester) or (is_tester and neighbor_is_dev):
